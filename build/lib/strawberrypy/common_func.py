...
def lattice_contraction(xcoord : np.ndarray, ycoord : np.ndarray, cutoff : float, nx : int, ny : int, atoms_uc : int):
    ...
    def within_range(current, trial):
        return True if (xcoord[current] - xcoord[trial]) ** 2 + (ycoord[current] - ycoord[trial]) ** 2 - cutoff ** 2 < 1e-6 else False

    for current in range(len(xcoord)):
        contraction.append([trial for trial in range(len(xcoord)) if within_range(current, trial)])

    return contraction

def average_over_radius(vals, xcoord, ycoord, cutoff, nx, ny, atoms_uc):
    """
