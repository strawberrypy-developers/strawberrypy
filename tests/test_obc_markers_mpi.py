...
from strawberrypy.example_models import (
    haldane_pythtb,
    haldane_tbmodels,
    kane_mele_pythtb,
    kane_mele_tbmodels,
...
def test_local_chern_marker_haldane_tbmodels_pythtb():
    ...
    hmodel_ptb_finite = Model(model=hmodel_ptb, spinful=False).make_finite(Lx=8, Ly=8)
    hmodel_tbm_finite = Model(model=hmodel_tbm, spinful=False).make_finite(Lx=8, Ly=8)

    if rank == 0:
        rdense_ptb = [r.toarray() for r in hmodel_ptb_finite.r]
        rdense_tbm = [r.toarray() for r in hmodel_tbm_finite.r]
        assert np.allclose(rdense_ptb, rdense_tbm)
        assert np.allclose(
            hmodel_ptb_finite.hamiltonian.toarray(),
            hmodel_tbm_finite.hamiltonian.toarray(),
        )
    ...
    if rank == 0:
        # The minus sign is a consistency bug fixed in StraWBerryPy > 0.3.2
    ...
    hmodel_ptb_finite.add_disorder_uniform(w=3, seed=seed)
    hmodel_tbm_finite.add_disorder_uniform(w=3, seed=seed)
...
def test_local_chern_marker_haldane_static():
    ...
    check_lcm = np.loadtxt("./check/lcm_uniform_disorder.check")
    check_positions = np.loadtxt("./check/positions_haldane_uniform.check")

    # Arbitrary order to check equality
    check_x, check_y, check_lcm = zip(
        *sorted(zip(check_positions[:, 0], check_positions[:, 1], check_lcm))
    )

    hmodel = haldane_tbmodels(delta=0.3, t=1, t2=0.15, phi=np.pi / 2)
    hmodel_finite = Model(model=hmodel, spinful=False).make_finite(Lx=8, Ly=8)

    # Set seed for disorder
    seed = 12345
    hmodel_finite.add_disorder_uniform(w=3, seed=seed)
...
def test_local_chern_marker_haldane_heterostructure_static():
    ...
    check_lcm = np.loadtxt("./check/lcm_heterostructure.check")
    check_positions = np.loadtxt("./check/positions_haldane_heterostructure.check")
    ...
    hmodel = haldane_tbmodels(delta=0.3, t=1, t2=0.15, phi=np.pi / 2)
    hmodel_finite = Model(model=hmodel, spinful=False).make_finite(Lx=16, Ly=8)

    hmodel_2 = haldane_tbmodels(delta=5.3, t=1, t2=0.15, phi=np.pi / 2)
    hmodel_2_finite = Model(model=hmodel_2, spinful=False).make_finite(Lx=16, Ly=8)

    # Build the heterostructure
    hmodel_finite.make_heterostructure(
        model2=hmodel_2_finite, direction=0, start=0, stop=7
    )
    ...
    if rank == 0:
        ...
        assert np.allclose(check_x, x)
        assert np.allclose(check_y, y)
        assert np.allclose(check_lcm, -np.array(lcm))
...
def test_local_chern_marker_haldane_vacancies_static():
    ...
    check_lcm = np.loadtxt("./check/lcm_vacancies.check")
    check_positions = np.loadtxt("./check/positions_vacancies.check")
...
def test_localization_marker_haldane_static():
    ...
    check_loc = np.loadtxt("./check/loc_haldane_uniform_disorder.check")
    check_positions = np.loadtxt("./check/positions_haldane_uniform.check")

    # Arbitrary order to check equality
    check_x, check_y, check_loc = zip(
        *sorted(zip(check_positions[:, 0], check_positions[:, 1], check_loc))
    )
...
def test_localization_marker_haldane_heterostructure_static():
    ...
    check_loc = np.loadtxt("./check/loc_haldane_heterostructure.check")
    check_positions = np.loadtxt("./check/positions_haldane_heterostructure.check")
...
def test_localization_marker_kane_mele_tbmodels_pythtb():
    ...
    # Build the finite model instance
    kmmodel_ptb_finite = Model(model=kmmodel_ptb, spinful=True).make_finite(Lx=8, Ly=8)
    kmmodel_tbm_finite = Model(model=kmmodel_tbm, spinful=True).make_finite(Lx=8, Ly=8)

    if rank == 0:
        rdense_ptb = [r.toarray() for r in kmmodel_ptb_finite.r]
        rdense_tbm = [r.toarray() for r in kmmodel_tbm_finite.r]
        assert np.allclose(rdense_ptb, rdense_tbm)
        assert np.allclose(
            kmmodel_ptb_finite.hamiltonian.toarray(),
            kmmodel_tbm_finite.hamiltonian.toarray(),
        )
    ...
    # Add onsite Anderson disorder
    kmmodel_ptb_finite.add_disorder_uniform(w=3, seed=seed)
    kmmodel_tbm_finite.add_disorder_uniform(w=3, seed=seed)
...
def test_localization_marker_kanemele_static():
    ...
    check_loc = np.loadtxt("./check/loc_kanemele_uniform_disorder.check")
...
def test_localization_marker_kanemele_heterostructure_static():
    ...
    check_loc = np.loadtxt("./check/loc_kanemele_heterostructure.check")
    check_positions = np.loadtxt("./check/positions_kanemele_heterostructure.check")
    ...
    kmmodel = kane_mele_tbmodels(rashba=0.3, esite=1, spin_orb=0.1)
    kmmodel_finite = Model(model=kmmodel, spinful=True).make_finite(Lx=16, Ly=8)

    kmmodel_2 = kane_mele_tbmodels(rashba=0.3, esite=5, spin_orb=0.1)
    kmmodel_2_finite = Model(model=kmmodel_2, spinful=True).make_finite(Lx=16, Ly=8)

    # Build the heterostructure
    kmmodel_finite.make_heterostructure(
        model2=kmmodel_2_finite, direction=0, start=0, stop=7
    )
