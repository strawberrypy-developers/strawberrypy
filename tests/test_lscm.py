...
def test_local_spin_chern_marker_kane_mele():
    ...
    # Check equality for the local spin Chern marker in the pristine case
    lscm_ptb = kmmodel_ptb_finite.local_spin_chern_marker()
    lscm_tbm = kmmodel_tbm_finite.local_spin_chern_marker()
    assert np.allclose(lscm_ptb, lscm_tbm)
    ...
    # Check equality for the local spin Chern marker in the disordered case
    lscm_ptb = kmmodel_ptb_finite.local_spin_chern_marker(macroscopic_average = True, cutoff = 1.5)
    lscm_tbm = kmmodel_tbm_finite.local_spin_chern_marker(macroscopic_average = True, cutoff = 1.5)
    assert np.allclose(lscm_ptb, lscm_tbm)