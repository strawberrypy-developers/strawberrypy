...
def test_spscn(L=6, r=1.0, e=3.0, spin_o=0.3, w=2.0):
    ...
    # Single Point Spin Chern Number (SPSCN) calculation for models created with both
    #   packages, for the same disorder configuration
    ...
    if DEBUG_MODE:
        print(
            ...
            + f"SPSCN : {spin_chern_pythtb['symmetric']}"
        )
        print(
            f"TBmodels package, supercell size L = {L}, disorder strength = {w}, "
            + f"SPSCN : {spin_chern_tbmodels['symmetric']}"
        )

    assert math.isclose(
        spin_chern_pythtb["asymmetric"], spin_chern_tbmodels["asymmetric"], abs_tol=1e-10
    )
    assert math.isclose(
        spin_chern_pythtb["symmetric"], spin_chern_tbmodels["symmetric"], abs_tol=1e-10
    )
