...
class Model():
    ...
    ----------
        ...
        file_spn :
            Whether the file `.spn` is provided if the model is obtained from Wannier functions generated by Wannier90 code starting from a :math:`\Gamma`-only ab-initio calculation. Default is ``False``.
        n_occ :
    ...
    def make_heterostructure(self, model2, direction : int = 0, start : int = 0, stop : int = 0):
        r"""
        Create a heterostructure by substituting the parameters of ``model2`` into the model. The system will be split in the direction ``direction`` starting from the unit cell whose index is ``start`` and stopping at the cell indexed as ``end``.
