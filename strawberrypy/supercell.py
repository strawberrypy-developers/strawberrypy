...
class Supercell(Model):
    ...
    It also can receive as input a model which is initialized as WannierBerri instance ``wannierberri.System_w90`` from the Wannier functions generated by Wannier90 code starting from a :math:`\Gamma`-only :math:`\textit{ab-initio}` calculation.
    It contains methods to add disorder and vacancies to the supercell and to calculate single-point topological invariants.

    Parameters
    ----------
        tbmodel : 
            Tight-binding model constructed from TBmodels or PythTB; Ab-initio tight-binding model inizialized as ``wannierberri.System_w90`` from WannierBerri.
        Lx :
    ...
    def single_point_chern(self, formula : str = 'both', return_ham_gap : bool = False):
        ...
        ----------
            ...
            return_ham_gap :
                If ``True``, returns the value of the gap between the highest occupied and the lowest unoccupied eigenstates of the Hamiltonian at :math:`\Gamma`-point. Default value is ``False``.
        Returns
        -------
            chern :
                Dictionary with results from ``asymmetric`` and/or ``symmetric`` formula for the single-point Chern number. 
            hamiltonian_gap : 
                Gap between the highest occupied and the lowest unoccupied eigenstates of the Hamiltonian at :math:`\Gamma`-point if ``return_ham_gap`` is ``True``.
        """
    ...
    def single_point_spin_chern(self, spin : str = 'down', formula : str = 'both', return_pszp_gap : bool = False, return_ham_gap : bool = False):
        ...
        ----------
            spin :
                Indicates which sector of the spin projected operator spectra is considered in the calculation of the single-point spin Chern number. If ``spin`` is ``up`` (``down``), eigenstates of spin projected operator with positive (negative) eigevalues are considered. Default value is ``down``.
            formula :
        ...
        -------
            spin_chern :
                Dictionary with results from ``asymmetric`` and/or ``symmetric`` formula for the single-point spin Chern number. 
            pszp_gap :
                Gap between positive and negative eigenvalues of the spin operator projected onto occupied states at :math:`\Gamma`-point if ``return_pszp_gap`` is ``True``.   
            hamiltonian_gap :
                Gap between the highest occupied and the lowest unoccupied eigenstates of the Hamiltonian at :math:`\Gamma`-point if ``return_ham_gap`` is ``True``.
        """
