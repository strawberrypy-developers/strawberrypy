...
class Supercell(Model):
    ...
    It also can receive as input a model which is initialized as ``wannierberri.System_w90`` from the Wannier functions generated by Wannier90 code starting from a :math:`\Gamma`-only ab-initio calculation.
    It contains methods to add disorder and vacancies to the supercell and to calculate single-point topological invariants.

    Parameters
    ----------
        tbmodel : 
            Tight-binding model constructed from TBmodels or PythTB; Ab-initio tight-binding model inizialized as ``wannierberri.System_w90``.
        Lx :
            Number of unit cells repeated along the :math:`\mathbf{a}_1` direction in the supercell. Give Lx=1 in case of ``wannierberri.System_w90`` instance.
        Ly :
            Number of unit cells repeated along the :math:`\mathbf{a}_2` direction in the supercell. Give Ly=1 in case of ``wannierberri.System_w90`` instance.
        spinful :
            Whether the model should be interpreted as spinful or not. Default is ``False``.

        Variables in case of ``wannierberri.System_w90`` instance
        ----------
            file_spn :
                Whether the file seedname.spn is provided if the model is obtained from Wannier functions generated by Wannier90 code starting from a :math:`\Gamma`-only ab-initio calculation. Default is ``False``.
            n_occ :
    ...
    def _pszp_matrix (self, u_n0 = None):
        r"""
        Returns the matrix of spin operator projected on the Hamiltonian occupied states at :math:`\Gamma`-point. Matrix element are :math:`\langle u_{n \Gamma} | \hat{S}_{z} | u_{m \Gamma} \rangle`, for :math:`| u_{n \Gamma} \rangle` occupied eigenstate.
        If the model is obtained from the Wannier functions generated by Wannier90 code starting from a :math:`\Gamma`-only ab-initio calculation and seedname.spn file is provided, spin matrix in the z-direction in the Wannier basis is constructed and projection onto occupied eigenstates is considered.
    ...
    def _dual_state(self, u_n0, u_nb, spin = None, n_sub = None):
        r"""
        Returns the "dual states" reported in Eq. (8) in Ref. (Ceresoli-Resta) for the calculation of the single-point Chern number and those in Eq. (10) in Ref. (SPSC) for the calculation of the single-point spin Chern number.  
        ...
        ----------
            ...
            spin :
                In case of ``spinful`` system, indicates which sector of spin projected operator spectra is considered in the calculation of the single-point spin Chern number.
                If ``spin`` is ``up`` (``down``), eigenstates of spin projected operator with positive (negative) eigevalues are considered.
            n_sub :
                Number of states used in the single-point invariant calculation. 
                If ``None``, the number of occupied eigenstates is used for the calculation of the single-point Chern number and the number of eigenstates of spin projected operator with eigenvalues of a given sign (:math:`\pm`) is used for the calculation of the single-point spin Chern number.
        """
        if n_sub is None:
            if self.spinful: 
                n_sub = self.n_occ//2
    ...
    def single_point_chern(self, formula : str = 'both', return_ham_gap : bool = False):
        r"""
        Evaluate the single-point Chern number derived in Ref. (Ceresoli-Resta).

        Parameters
        ----------
            formula :
                Formula used for the calculation of the single-point Chern number among two possibilities: the ``asymmetric`` one is originally derived in Ref. (Ceresoli-Resta) and uses right-hand discrete derivative; the ``symmetric`` one uses symmetric derivative centered in :math:`\Gamma`-point and converges faster to the exact value with the supercell size. Default value is ``both``. 
            return_ham_gap :
                If ``True``, returns the value of the gap between the highest occupied and the lowest unoccupied eigestates of the Hamiltonian at :math:`\Gamma`-point. Default value is ``False``.
        """
    ...
    def single_point_spin_chern(self, spin : str = 'down', formula : str = 'both', return_pszp_gap : bool = False, return_ham_gap : bool = False):
        r"""
        Evaluate the single-point spin Chern number derived in Ref. (SPSCN).

        Parameters
        ----------
            spin :
                Indicates which sector of spin projected operator spectra is considered in the calculation of the single-point spin Chern number. If ``spin`` is ``up`` (``down``), eigenstates of spin projected operator with positive (negative) eigevalues are considered. Default value is ``down``.
            formula :
                Formula used for the calculation of the single-point spin Chern number among two possibilities reported in Ref. (SPSCN): the ``asymmetric`` one uses right-hand discrete derivative; the ``symmetric`` one uses symmetric derivative centered in :math:`\Gamma`-point and converges faster to the exact value with the supercell size. Default value is ``both``. 
            return_pszp_gap :
                If ``True``, returns the value of the gap between positive and negative eigenvalues of the spin operator projected onto occupied states at :math:`\Gamma`-point. Default value is ``False``.
            return_ham_gap :
                If ``True``, returns the value of the gap between the highest occupied and the lowest unoccupied eigestates of the Hamiltonian at :math:`\Gamma`-point. Default value is ``False``.
        """
