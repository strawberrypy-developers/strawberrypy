...
class FiniteModel(Model):
    r"""
    A class describing a finite (OBC) model built from either TBmodels or PythTB instances.
    It also can receive as input a model which is initialized as WannierBerri instance :python:`wannierberri.System_w90` from the Wannier functions generated by Wannier90 code starting from an *ab initio* calculation.
    It contains methods to add disorder and vacancies to the finite model, calculate local topological markers and the localization marker.
    ...
    ----------
        ...
        file_spn :
            Whether the file seedname.spn is provided if the model is obtained from Wannier functions generated by Wannier90 code starting from an *ab initio* calculation. Default is :python:`False`.
        n_occ :
            Number of occupied states. This information is required if the model is obtained from Wannier functions generated by Wannier90 code starting an *ab initio* calculation. In all the other cases the number of occupied states can be set and if :python:`None` half-filling is automatically assumed.
    """

    def __init__(self, tbmodel, Lx : int = 1, Ly : int = 1, spinful : bool = False, file_spn : bool = False, n_occ : int = None):
    ...
    def local_chern_marker(self, direction : int = None, start : int = 0, return_projector : bool = False, input_projector : np.ndarray = None, macroscopic_average : bool = False, cutoff : float = 0.8,  bare_marker : bool = False, smearing_temperature : float = 0.0, fermidirac_cutoff : float = 0.1, n_tba : int = 0):
        r"""
        ...
        ----------
            direction :
                Direction along which the local Chern marker is computed. Default is :python:`None` (returns the marker on the whole lattice). Allowed directions are ``0`` (:math:`\mathbf{a}_1` direction), and ``1`` (:math:`\mathbf{a}_2` direction).
            start :
            ...
            input_projector :
                Possibility to provide the ground state projector to be used in the calculation as input. Default is :python:`None`, which means that the ground state projector is computed from the model.
            macroscopic_average :
            ...
            bare_marker :
                If :python:`True` returns the bare local Chern marker, without taking local trace and averaging. Default is :python:`False`.
        ...
        -------
            ...
            local_c :
                The bare local Chern marker :math:`C(\mathbf{r})` returned without averaging if :python:`bare_marker == True`.
        """
        ...
        if input_projector is None:
            ...
            gs_projector = contract("ji,ki->jk", utils.smearing(eigenvecs[:, :rank], eigenvecs, eigenvals, smearing_temperature, mu) * eigenvecs[:, :rank], eigenvecs[:, :rank].conjugate())
        else:
    ...
    def localization_marker(self, direction : int = None, start : int = 0, return_projector : bool = None, input_projector : np.ndarray = None, macroscopic_average : bool = False, cutoff : float = 0.8, n_tba : int = 0):
        r"""
        ...
        ----------
            direction :
                Direction along which the localization marker is computed. Default is :python:`None` (returns the marker on the whole lattice). Allowed directions are ``0`` (:math:`\mathbf{a}_1` direction), and ``1`` (:math:`\mathbf{a}_2` direction).
            start :
            ...
            n_tba :
                Number of states to be added relative to the a priori set filling (defined by the :python:`n_occ` parameter or the default half-filling).
        ...
        if input_projector is None:
            ...
            # Build the ground state projector
            rank = self.n_occ + n_tba
            gs_projector = contract("ji,ki->jk", eigenvecs[:, :rank], eigenvecs[:, :rank].conjugate())
        else:
            gs_projector = input_projector
    ...
    def local_spin_chern_marker(self, direction : int = None, start : int = 0, return_projector : bool = None, input_projector = None, macroscopic_average : bool = False, cutoff : float = 0.8,  bare_marker : bool = False, smearing_temperature : float = 0, fermidirac_cutoff : float = 0.1, n_tba : int = 0):
        r"""
        ...
        ----------
            direction :
                Direction along which the local spin-Chern marker is computed. Default is :python:`None` (returns the marker on the whole lattice). Allowed directions are ``0`` (:math:`\mathbf{a}_1` direction), and ``1`` (:math:`\mathbf{a}_2` direction).
            start :
            ...
            input_projector :
                Possibility to provide the list of projectors :math:`[\mathcal P_+, \mathcal P_-]` to be used in the calculation as input. Default is :python:`None`,  which means that they are computed from the model.
            macroscopic_average :
            ...
            bare_marker :
                If :python:`True` returns the bare local individual Chern markers, without taking local trace and averaging. Default is :python:`False`.
            smearing_temperature :
        ...
        --------
            ...
            local_cp, local_cm :
                The list of bare local individual Chern markers [:math:`C_+(\mathbf{r})`, :math:`C_-(\mathbf{r})`], returned if :python:`bare_marker == True`.
        """
        ...
        if input_projector is None:
            ...
            lowerproj = contract("ki,ji->kj", evecs[:, :nsub], utils.smearing(evecs[:, :nsub], eigenvecs, eigenvals, smearing_temperature, mu) * evecs[:, :nsub].conjugate())
            higherproj = contract("ki,ji->kj", evecs[:, nsub:], utils.smearing(evecs[:, nsub:], eigenvecs, eigenvals, smearing_temperature, mu) * evecs[:, nsub:].conjugate())
        else:
        ...
        if bare_marker: return np.array([np.diag(chern_operator_plus), np.diag(chern_operator_minus)])
    ...
    def local_z2_marker(self, direction : int = None, start : int = 0, return_projector : bool = None, input_projector = None, macroscopic_average : bool = False, cutoff : float = 0.8, bare_marker : bool = False, smearing_temperature : float = 0, fermidirac_cutoff : float = 0.1, trial_projections = None, n_tba : int = 0):
        r"""
        ...
        ----------
            ...
            input_projector :
                Possibility to provide the list of projectors :math:`[\mathcal P_1, \Theta\mathcal P_1\Theta^{-1}]` to be used in the calculation as input. Default is :python:`None`,  which means that they are computed from the model.
            macroscopic_average :
        ...
        --------
            ...
            local_c1, local_c2 :
                The bare local individual Chern markers :math:`C_1(\mathbf{r})` and :math:`C_2(\mathbf{r})`, returned if :python:`bare_marker == True`.
        """
        ...
        if input_projector is None:
            ...
            projector = contract("ik,ij->kj", vectors, (utils.smearing(vectors.T, eigenvecs, eigenvals, smearing_temperature, mu) * vectors.T.conjugate()).T)
            trprojector = contract("ik,ij->kj", tr_vectors, (utils.smearing(tr_vectors.T, eigenvecs, eigenvals, smearing_temperature, mu) * tr_vectors.T.conjugate()).T)
        else:
